...
const {
  ...
  get_all,
  get_by_id,
} = require("../controllers/branches_controller");

router.post("/create", async (req, res) => {
  const { name, description, branch_code, parent_branch_id } = req.body;
  const result = await create(name, description, branch_code, parent_branch_id);
  if (result.valid) res.status(200).send(result);
  else res.status(404).send(result);
});

router.post("/update", async (req, res) => {
  const { id, name, description, branch_code, parent_branch_id } = req.body;
