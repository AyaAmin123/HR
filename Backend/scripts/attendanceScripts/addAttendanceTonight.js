...
async function insertRecords(from, to) {
  try {
    let jobName = `اضافة حضور من ${from.split("T")[0]} ل ${to.split("T")[0]}`;
    ...
    for (const employee of employees) {
      ...
      while (fromDate <= toDate) {
        let date = fromDate.toISOString().split("T")[0];
        if (
          employee.Branch &&
          employee.Branch.name &&
          branches_9.includes(employee.Branch.name.trim())
        ...
        if (att === null)
          await Attendance.create({
            ...
            forgiveness_evening_time,
            forgiveness_morning_time,
            createdAt: new Date(`${date} 09:00:00`),
            updatedAt: new Date(`${date} 09:00:00`),
          });
        // }
...
async function updateAttendanceAfterRequestHasBeenMade(employee, request_date) {
  let timeFrom = "09:00:00",
    timeTo = "17:00:00",
    forgiveness_evening_time = 15,
    forgiveness_morning_time = 15;
  ...
  const {
    ...
    request_id,
    official_holiday_id,
  } = await get_planned_in_and_out(employee.id, request_date, timeFrom, timeTo);
  let att = await Attendance.findOne({
    where: {
      emp_id: employee.id,
      current_date: new Date(`${request_date} 00:00:00`),
    },
  });
  if (att) {
    await Attendance.update(
      {
        ...
        forgiveness_evening_time,
        forgiveness_morning_time,
        createdAt: new Date(`${request_date} 09:00:00`),
        updatedAt: new Date(`${request_date} 09:00:00`),
      },
      {
        where: {
          emp_id: employee.id,
