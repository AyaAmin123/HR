...
async function calculateLatnessFromTo(from, to) {
  let tempFrom = new Date(from.toISOString());

  try {
    let day = "";
    while (tempFrom <= to) {
      day = tempFrom.toISOString().split("T")[0];
      ...
      if (data && data.length !== 0) {
        for (let {
          ...
          //TODO: check if have vacations or deduct tempFrom salary
        }
        console.log(
          `[${new Date(
            new Date() + "UTC"
